...
exports.login =async(req,res)=>{
    try{
        ...
        if(!email || !password){
            return res.status(400).render('login',({msg:'Please Enter the Email and Password',msg_type:'error'}));
        }

        db.query('select * from users where email=?',[email],async(error,result)=>{
            console.log(result);
            if(result.length<=0){
                return res.status(401).render('login',({msg:'Please Enter a valid email',msg_type:'error'}))
            }else{
                if(!(await bcrypt.compare(password,result[0].PASS))){
                    return res.status(401).render('login',({msg:'Please Enter the Email and Password',msg_type:'error'}));
                }else{
                    ...
                    res.cookie("eshwar",token,cookieOptions);
